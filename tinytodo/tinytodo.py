...
class StoppedServer:
    def get(self, _):
        raise Exception('No server running! Use `start_server()`!')
...
def start_server(filename, port = 8080):
    global server
    if server.stopped():
        server = Server(str(port), filename)
    else:
...
def get_lists_printer(user):
    def inner(list_of_lists):
        ...
        if len(list_of_lists) == 0:
            ...
            return ('Lists: %s' % '\n'.join([str(List(lst)) for lst in list_of_lists[:MAX_LISTS]])
                    + ('\n... and %d more' % (len(list_of_lists) - MAX_LISTS) if len(list_of_lists) > MAX_LISTS else ''))
...
def create_task(user, list_id, name):
    ...
    return server.post(url, data), lambda _ : 'Created task on list ID %s' % list_id
